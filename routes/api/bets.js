...
// Route to fetch user information
router.post('/betDuplication', async (req, res) => {

  try {
    const transaction_id = req.body['transaction_id'];
    const session_id      = req.body['session_id'];
    
    const checkDuplicate = await SlotGames.find({
      $and: [
    ...
    if (checkDuplicate.length === 0) {
      ...
      return res.status(200).json({
        error_code: "INTERNAL_ERROR",
        error_description: "This transaction is already exist in our database",
...
router.get('/betting-logs', async (req, res) => {
  try {
    ...
    const user = await SlotGames.find({ player_id: player_id }).sort({ _id: -1 }).limit(20); // Use an object with the email property as the filter
    // console.log(user);
    res.json(user);
  } catch (err) {
    console.error(err);
