...
router.get("/create_country", async(req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Countries data inserted successfully.' });
  } catch (err) {
    console.error('Error inserting countries data:', err);
    res.status(500).json({ message: 'Failed to insert countries data.' });
  }
