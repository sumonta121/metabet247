...
const AffiliateList = () => {
  ...
  const getAllUser = () => {
    fetch(`${apiConfig.baseURL}/api/agent/getdataAffiliate`, {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const referrerid = userInfo.user_id;
    ...
    fetch(
      `${apiConfig.baseURL}/api/agent/paginatedAffiliate?page=${currentPage.current}&limit=${limit}&referrerid=${referrerid}`,
      {
        method: "GET",
      }
    )
